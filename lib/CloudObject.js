...
Cloudobj.prototype._upload = function(callback) {
  ...
  this._db.insert(this._docObj, this._docName, function(err, body) {
    if (err) {
...
Cloudobj.prototype._download = function(callback, nochange) {
  var target = this;
  this._db.get(this._docName, function(err, body) {
    if (err) {
...
var onWrite = function(cloudObj, callback) {
  if (callback) callback();
  return; // DISABLE onWrite automatic writing
  NULLWRITE("Write lock?", cloudObj._writing, cloudObj._docName);
  if (cloudObj._writing) {
    NULLWRITE('Writing lock engaged', cloudObj._docName);
    cloudObj._queue.push([cloudObj, callback]);
    return;
  }
  cloudObj._writing = true;
  NULLWRITE('Writing with rev', cloudObj._rev);
  NULLWRITE('NEW WRITE:');
  if (!cloudObj._isReady) {
    NULLWRITE('No sync, CloudObject not ready yet');
    cloudObj._writing = false;
    return;
  }
  /*if (!cloudObj._docObj._rev) { 
    NULLWRITE('Write skipped. No _rev found.');
    throw new Error('No _rev found.');
    return;
  }*/
  if (!cloudObj._db) { 
    NULLWRITE('Write skipped. No database found.');
    cloudObj._writing = false;
    return;
  }
  NULLWRITE('Writing:', cloudObj._docObj);
  cloudObj._db.insert(cloudObj._docObj, cloudObj._docName, function(err, body) {
    if(err) {
      console.error(err);
      cloudObj._writing = false;
      if (callback) callback(err);
    } else {
      NULLWRITE('Upload complete for object', cloudObj._docName);
      NULLWRITE('New _rev:', body.rev);
      cloudObj._rev = body.rev;
      cloudObj._docObj._rev = body.rev;
      cloudObj._writing = false;
      if (cloudObj._queue.length > 0) {
        var nxt = cloudObj._queue.splice(0,1);
        NULLWRITE('Lock trade');
        onWrite(nxt[0], nxt[1]);
      }
      if (callback) callback();
    }
...
var onRead = function(cloudObj) {
};

var fireReady = function(cloudObj) {
  var cb = cloudObj._readyCallbacks;
  for (var i = 0; i < cb.length; i++) {
    cb[i]();
  }
  cloudObj._readyCallbacks = [];
};

var replaceLocal = function(cloudObj) {
  return; // DISABLE onWrite automatic writing
  var database = cloudObj._db;
  var documentName = cloudObj._docName;
  database.get(documentName, function(err, body) {
    if (err && err.error == 'not_found') {
      NULLWRITE('Object not found in db. Creating...');
      cloudObj._isReady = true;
      onWrite(cloudObj, function(err) {
        if (err) {
          NULLWRITE('There was an error, so cloud object is not ready');
          cloudObj._isReady = false;
        } else {
          fireReady(cloudObj);
        }
        NULLWRITE('Sync done.');
      ...
      for (var property in body) {
        var pIndex = cloudObj._updatedProperties.indexOf(property);
        if (body.hasOwnProperty(property) && pIndex < 0) {
          NULLWRITE('Overwriting', property, pIndex);
          cloudObj._docObj[property] = body[property];
        }
      }
      NULLWRITE('Done fetching');
      NULLWRITE('Going to set');
      cloudObj._isReady = true;
      NULLWRITE('cloudObj ready ');
      onWrite(cloudObj, function(err) {
        if (err) {
          NULLWRITE('There was an error, so cloud object is not ready');
          cloudObj._isReady = false;
