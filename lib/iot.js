...
exports.register = function(device, orgname, interval, devUID) {
  ...
  var sendMessage = function(device, orgname) {
    ...
    function variableCallback(jsonstr, variable, vObj, substring) {
    ...
    for (variable in variables) {
      ...
      if (vObj.type == 'script') {
        value = null;
        if (!vObj.history) vObj.history = [];
        if (vObj.script) {
          var s = new Sandbox();
          var runscript  = 'onmessage = function(message) {\n';
              runscript += vObj.script + '\n';
              runscript += '_VAL = JSON.parse(message);\n';
              runscript += 'postMessage(value(_VAL.history, _VAL.properties));\n';
              runscript += '};\n';
          console.log('Running', runscript);
          s.run(runscript, function() {});
          s.on('message', function(output) {
            console.log('Got output', output);
            vObj.history.push(output);
            if (vObj.history.length > 10) {
              vObj.history.shift();
            }
            variableCallback(jsonstr, variable, vObj, output);
          });
          s.postMessage(JSON.stringify({"history":vObj.history, "properties":{}}));
        }
        ...
        variableCallback(jsonstr, variable, vObj, substring);
